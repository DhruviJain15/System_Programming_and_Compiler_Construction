...
#print(content_list);
kw=['auto','double','int','struct','break','else','long','switch','case','enum','register','typedef','char','extern','return','union','continue','for','signed','void',
...
for word in content_list:
	...
	if word in kw:
		...
		if (ord(word)>=33 and ord(word)<=47) or (ord(word)>=58 and ord(word)<=64) or (ord(word)>=123 and ord(word)<=126):
